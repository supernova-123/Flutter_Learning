...
class _HomePageState extends State<HomePage>
  ...
  void initState() {
    ...
    _controller = AnimationController(
      ...
      duration: Duration(seconds: 2), // 每多久一次循环
          ...
          children: [
            SlideTransition(
              position: 
                Tween( // Tween配置运动参数
                  begin: Offset(0, 0),
                  end: Offset(0, -2), // 向右、下移动0.5*width和0.5*height
                )
                .chain( // 配置动画效果
                  CurveTween(curve: Curves.bounceInOut)
                )
                .chain(
                  CurveTween(curve: Interval(0.2, 0.8)) // 交错动画：从2秒时间的20%开始动，到时间80%停止，中间不动
                )
                .animate(_controller), // 配置controller
              child: Container(
                width: 100,
                height: 100,
                color: Colors.red,
